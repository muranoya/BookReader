...
BenchmarkDialog::BenchmarkDialog(QWidget *parent)
    ...
    , nearest_text(new QLabel()),  nearest_lcd(new QLCDNumber())
    , bilinear_text(new QLabel()), bilinear_lcd(new QLCDNumber())
    , bicubic_text(new QLabel()),  bicubic_lcd(new QLCDNumber())
...
void
BenchmarkDialog::startButtonClicked()
{
    const int img_size = comboBox->currentText().toInt();
    qint64 elapsed;
...
void
BenchmarkDialog::rejectedButtonClicked()
{
